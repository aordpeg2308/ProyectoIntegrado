...
<div class="flex items-center justify-center py-12">
    <div class="bg-[#f6d6ba]/80 shadow-lg rounded-xl p-8 w-full max-w-lg">
        <h2 class="text-2xl font-bold text-center text-[#2e2d55] mb-6">Editar juego</h2>

        <form action="{{ route('juegos.update', $juego) }}" method="POST" novalidate>
            @csrf
            @method('PATCH')
            ...
            <div class="mb-4">
                ...
                <select name="tipo" id="tipo"
                    ...
                    <option value="Juego de mesa" {{ old('tipo', $juego->tipo) == 'Juego de mesa' ? 'selected' : '' }}>Juego de mesa</option>
                    <option value="Manual de rol" {{ old('tipo', $juego->tipo) == 'Manual de rol' ? 'selected' : '' }}>Manual de rol</option>
                </select>
                @error('tipo')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
                ...
                <select name="user_id" id="user_id"
                    ...
                    @foreach ($usuarios as $usuario)
                        <option value="{{ $usuario->id }}"
                            {{ old('user_id', $juego->user_id) == $usuario->id ? 'selected' : '' }}>
