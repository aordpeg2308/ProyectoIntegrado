<div class="max-w-5xl mx-auto py-6 space-y-6 text-[#2e2d55]">
    <form method="GET" action="{{ route('home') }}">
        <div class="relative w-full max-w-md mx-auto">
            <input type="text" name="buscar_organizadas" placeholder="Buscar partidas organizadas..."
                value="{{ request('buscar_organizadas') }}"
    ...
    <div class="bg-[#f6d6ba]/80 rounded-xl shadow-lg overflow-x-auto w-full">
        ...
        @if ($partidasOrganizadas->isEmpty())
            <p class="text-center p-6 text-lg font-semibold">No has organizado partidas.</p>
        @else
                        ...
                        <tr class="hover:bg-[#f6d6ba]/60 transition">
                            ...
                            <td class="p-4 flex gap-2">
                                ...
                                <form method="POST" action="{{ route('partidas.destroy', $partida) }}"
                                    onsubmit="return confirm('¿Seguro que quieres eliminar esta partida?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
